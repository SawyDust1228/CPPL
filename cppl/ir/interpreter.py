...
def _parse_literal(value: Union[int, str]) -> int:
...
class Interpreter:
    ...
    def __init__(
        ...
        self._check_recursive_instances()
        self.top = self._select_top(top)
        self.base_dir = Path(base_dir) if base_dir is not None else Path.cwd()
        self._initializing = False
        self._root = self._build_instance(self._modules_by_name[self.top], self.top)
        self.reset_state()
    ...
    def evaluate(self, inputs: Optional[Mapping[str, int]] = None) -> Dict[str, int]:
        ...
        for _ in range(_MAX_DELTA_CYCLES):
            self._evaluate_instance(self._root, self._root.inputs)
            edge_updates = self._collect_edge_updates(self._root)
            self._record_clock_levels(self._root)
            if not edge_updates:
    ...
    def peek(self, path: str) -> int:
        ...
        instance, value_name = self._resolve_value_path(path)
    ...
    def peek_memory(self, path: str) -> tuple[int, ...]:
        ...
        instance, memory_name = self._resolve_value_path(path)
        matches = [
            memory for memory in instance.memories.values()
    ...
    def reset_state(self) -> None:
        ...
        self._reset_instance(self._root)
    ...
    def _select_top(self, requested: Optional[str]) -> str:
        ...
        roots = [module.name for module in self.modules if module.name not in instantiated]
        if len(roots) == 1:
            return roots[0]
        if not roots:
    ...
    def _check_recursive_instances(self) -> None:
        visiting: List[str] = []
        visited: set[str] = set()
    ...
    def _build_instance(self, module: Module, path: str) -> _InstanceState:
        inputs = {
            name: 0
        ...
        for index, op in enumerate(module.body):
            if isinstance(op, RegOp):
                ...
                values = self._load_memory(op, path)
    ...
    def _load_memory(self, op: MemOp, instance_path: str) -> List[int]:
        values = [0] * op.depth
        if not op.initFile:
            return values
        ...
        for token in tokens:
            ...
            values[address] = _truncate(value, op.width)
            address += 1
        return values

    def _reset_instance(self, instance: _InstanceState) -> None:
        ...
        for index, memory in instance.memories.items():
            memory.values[:] = self._load_memory(memory.op, instance.path)
            instance.previous_clocks[index] = 0
        instance.env.clear()
        instance.outputs.clear()
        for child in instance.children.values():
            self._reset_instance(child)
    ...
    def _apply_inputs(self, inputs: Mapping[str, int]) -> None:
        ...
        for name, value in inputs.items():
            ...
            normalized[name] = _truncate(value, port.width)
    ...
    def _evaluate_instance(
        ...
        *,
        ...
        env = {
            name: _truncate(input_values[name], port.width)
            for name, port in module.ports.items()
        ...
        while pending:
            ...
            for index, op in pending:
                if isinstance(op, ConstantOp):
                    env[op.id] = _truncate(_parse_literal(op.value), op.width)
                elif isinstance(op, RegOp):
                    env[op.id] = instance.registers[index]
                elif isinstance(op, UnaryOp) and self._args_ready(op.args, env):
                    env[op.id] = self._eval_unary(op, env, widths)
                elif isinstance(op, BinaryOp) and self._args_ready(op.args, env):
                    env[op.id] = self._eval_binary(op, env, widths, instance.path)
                elif isinstance(op, VariadicOp) and self._args_ready(op.args, env):
                    env[op.id] = self._eval_concat(op, env, widths)
                elif isinstance(op, MuxOp) and self._args_ready(op.args, env):
                    env[op.id] = env[op.args[1]] if env[op.args[0]] else env[op.args[2]]
                elif isinstance(op, CastOp) and self._args_ready(op.args, env):
                    env[op.id] = self._eval_cast(op, env, widths)
                elif isinstance(op, ExtractOp) and self._args_ready(op.args, env):
                    env[op.id] = _truncate(env[op.args[0]] >> op.lowBit, op.width)
                elif isinstance(op, MemOp) and self._mem_read_args_ready(op, env):
                    memory = instance.memories[index]
                    for output_id, (addr_ref, enable_ref) in zip(op.id, op.reads):
                        if env[enable_ref]:
                            address = env[addr_ref]
                            self._check_address(address, op, instance.path, "read")
                    ...
                    child_inputs = {
                        name: env[ref]
                        for name, ref in op.args.items()
                        if ref in env
                    }
                    child.inputs.update(child_inputs)
                    child_outputs = self._evaluate_instance(
                        child, child_inputs, allow_partial=True
                    )
                    output_names = [
                        name for name, port in child.module.ports.items()
        ...
        outputs = {
            name: env[ref]
            for name, ref in output_op.args.items()
            if ref in env
        }
        instance.env = env
    ...
    @staticmethod
    def _args_ready(args: object, env: Mapping[str, int]) -> bool:
        return all(arg in env for arg in args)

    @staticmethod
    def _mem_read_args_ready(op: MemOp, env: Mapping[str, int]) -> bool:
    ...
    def _eval_binary(
        ...
        result_width = widths[op.id].width
        lhs_signed = _as_signed(lhs, operand_width)
        rhs_signed = _as_signed(rhs, operand_width)
        ...
        if op.op == "add":
            ...
            result = lhs // rhs if self._check_divisor(rhs, op.op, instance_path) else 0
        elif op.op == "div_s":
            result = (
                _signed_div(lhs_signed, rhs_signed)
                if self._check_divisor(rhs_signed, op.op, instance_path) else 0
            )
        elif op.op == "mod_u":
            result = lhs % rhs if self._check_divisor(rhs, op.op, instance_path) else 0
        elif op.op == "mod_s":
            if self._check_divisor(rhs_signed, op.op, instance_path):
                quotient = _signed_div(lhs_signed, rhs_signed)
            ...
            result = (-1 if lhs_signed < 0 else 0) if rhs >= operand_width else lhs_signed >> rhs
    ...
    def _check_divisor(self, value: int, op_name: str, instance_path: str) -> bool:
        if value == 0:
    ...
    def _eval_concat(
        ...
        return _truncate(result, widths[op.id].width)
    ...
    def _eval_cast(
        ...
        value = env[op.args[0]]
        if op.op == "sext":
            value = _as_signed(value, widths[op.args[0]].width)
        return _truncate(value, op.width)

    def _collect_edge_updates(self, instance: _InstanceState) -> List[object]:
        updates: List[object] = []
        env = instance.env
        for index, op in enumerate(instance.module.body[:-1]):
            ...
            if isinstance(op, RegOp):
                if op.reset and env[op.reset]:
                    next_value = _truncate(
                        _parse_literal(op.resetValue),
        ...
        for child in instance.children.values():
            updates.extend(self._collect_edge_updates(child))
        return updates

    def _record_clock_levels(self, instance: _InstanceState) -> None:
        for index, op in enumerate(instance.module.body[:-1]):
            if isinstance(op, (RegOp, MemOp)):
                instance.previous_clocks[index] = instance.env[op.clock] & 1
        for child in instance.children.values():
            self._record_clock_levels(child)

    @staticmethod
    def _check_address(address: int, op: MemOp, path: str, action: str) -> None:
        if address < 0 or address >= op.depth:
    ...
    def _resolve_value_path(self, path: str) -> tuple[_InstanceState, str]:

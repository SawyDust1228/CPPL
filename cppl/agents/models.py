    ...
    def resolve(self) -> "ResolvedAgentConfig":
        ...
        resolved = ResolvedAgentConfig(
            max_parallelism=(
                ...
                else _env_int("CPPL_AGENT_MAX_PARALLELISM", 4)
            ),
            context_window_tokens=(
                self.context_window_tokens
                if self.context_window_tokens is not None
                else _env_int("CPPL_AGENT_CONTEXT_WINDOW", 32768)
            ),
            output_tokens=output_tokens,
            safety_margin_tokens=(
                self.safety_margin_tokens
                if self.safety_margin_tokens is not None
                else _env_int("CPPL_AGENT_SAFETY_MARGIN", 2048)
            ),
            cache_enabled=(
                self.cache_enabled
                if self.cache_enabled is not None
                else _env_bool("CPPL_AGENT_CACHE_ENABLED", True)
            ),
            cache_dir=Path(cache_dir),
            fail_fast=(
                self.fail_fast
                if self.fail_fast is not None
                else _env_bool("CPPL_AGENT_FAIL_FAST", True)
            ),
            transport_retries=(
                self.transport_retries
                if self.transport_retries is not None
                else _env_int("CPPL_AGENT_TRANSPORT_RETRIES", 2)
            ),
...
class CompilationReport:
    ...
    def success(self) -> bool:
        return (
            self.design_error is None
            and all(report.success for report in self.module_reports.values())
        )
